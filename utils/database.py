...
class DatabaseConnector:
    ...
    def rename_folder(self, old_name: str, new_name: str) -> bool:
        """Rename a folder and update all related rules"""
        try:
            # Update folder name
            folder = self.db.query(Folder).filter(Folder.name == old_name).first()
            if folder:
                folder.name = new_name

                # Update all rules in this folder
                rules = self.db.query(Rule).filter(Rule.folder == old_name).all()
                for rule in rules:
                    rule.folder = new_name

                # Update child folders
                child_folders = self.db.query(Folder).filter(
                    Folder.name.like(f"{old_name}/%")
                ).all()
                for child in child_folders:
                    child.name = child.name.replace(old_name, new_name, 1)
                    child.parent_folder = child.parent_folder.replace(old_name, new_name, 1)
            ...
            self.db.rollback()
            raise Exception(f"Failed to rename folder: {str(e)}")

    def move_rule(self, rule_id: int, target_folder: str) -> bool:
        """Move a rule to a different folder"""
        try:
            rule = self.db.query(Rule).filter(Rule.id == rule_id).first()
            if rule:
                rule.folder = target_folder
                self.db.commit()
                return True
            return False
        except Exception as e:
            self.db.rollback()
            raise Exception(f"Failed to move rule: {str(e)}")